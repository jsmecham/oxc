...
impl ScopedWalker {
    ...
    ///
    /// Returns `None` if all target paths were filtered out by ignore rules.
    /// This allows the caller to handle "no files" before starting the walk.
    pub fn prepare_root_targets(
        &self,
        config_dir: Option<&Path>,
        ignore_patterns: &[String],
    ) -> Result<Option<RootTargets>, String> {
        ...
        Ok(Some(RootTargets { target_paths, matchers }))
    }
